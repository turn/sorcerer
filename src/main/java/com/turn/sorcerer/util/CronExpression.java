...
*
* Adapted from https://github.com/frode-carlsen/cron with minimal changes
*/
...
public class CronExpression {
	...
	public CronExpression(String expr) {
		...
		if (parts.length != 6) {
			throw new IllegalArgumentException(String.format("Invalid cronexpression [%s], expected %s arguments, got %s"
	...
	abstract static class BasicField {
		...
		private static final Pattern CRON_FELT_REGEXP = Pattern
				.compile("(?:\n"                                             // start of group 1
						+ "   (?:(?<all>\\*)|(?<ignorer>\\?)|(?<last>L))\n"  // globalt flag (L, ?, *)
						+ " | (?<start>[0-9]{1,2}|[a-z]{3,3})\n"             // or start number or symbol
						+ "      (?:\n"                                      // start of group 2
						+ "         (?<mod>L|W)\n"                           // modifier (L,W)
						+ "       | -(?<end>[0-9]{1,2}|[a-z]{3,3})\n"        // or end nummer or symbol (in range)
						+ "      )?\n"                                       // end of group 2
						+ ")\n"                                              // end of group 1
						+ "(?:(?<inkmod>/|\\#)(?<ink>[0-9]{1,7}))?\n"        // increment and increment modifier (/ or \\#)
